...
class Submit2 : Fragment(), View.OnClickListener, iOnBackPressed {
    ...
    fun init() {
        ...
        if(listBranch[1].branchName=="NP"){
            ...
            edittext1.setText("Section III - Not Permitted")
        }else{
            edittext1.isEnabled = true
            edittext1.hint = "Section III : Range "+resources.getString(R.string.special_symbol)+" ${listBranch.get(1).branchName}"
        }
        ...
        when (v!!.id) {
            R.id.submitBtn -> {
                if(validation()) {
                    ...
                    intent.putExtra("Section", "III")
                    intent.putExtra("Temperature", listBranch.get(1).branchName)
    ...
    private fun validation(): Boolean {
        return when {
            !edittext1.text.toString()
                .isNullOrEmpty() && listBranch[1].branchName != "NP" &&  edittext1.text.toString()
                .toDouble()  > listBranch[1].branchName.toDouble()-> {
