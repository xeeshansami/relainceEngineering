...
class Submit3 : Fragment(), View.OnClickListener, iOnBackPressed {
    ...
    fun init() {
        ...
        if(listBranch[2].branchName=="NP"){
            ...
            edittext1.setText("Section VIII-1 - Not Permitted")
        }else{
            edittext1.isEnabled = true
            edittext1.hint = "Section VIII : Range "+resources.getString(R.string.special_symbol)+" ${listBranch.get(2).branchName}"
        }
        ...
        when (v!!.id) {
            R.id.submitBtn -> {
                if(validation()) {
                    ...
                    intent.putExtra("Section", "VIII")
                    intent.putExtra("Temperature", listBranch.get(2).branchName)
    ...
    fun getCalcValue(): ArrayList<Int> {
        ...
        if(tempVal<temperaturValues!!.get(0)){
            pair.add(temperaturValues!!.get(0))
            return pair
        }else {
    ...
    private fun validation(): Boolean {
        return when {
            !edittext1.text.toString()
                .isNullOrEmpty() && listBranch[2].branchName != "NP" &&  edittext1.text.toString()
                .toDouble()  > listBranch[2].branchName.toDouble() -> {
